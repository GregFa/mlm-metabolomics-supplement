...
begin 
# Size of column of mXspiro is greater because of more sites	
if (@isdefined mXspiro)
	if radio_Xspiro
		mXspiro
	else
...
if (@isdefined mXcopd)
	...
	copdZI = getcoeffs_mlm(copd.mY, mXcopd, ZI, hasXIntercept = true, hasZIntercept = false);
...
begin
	if @isdefined copd
		...
		psp_copd_x_lims = [xlims(psp_copd)[1] xlims(psp_copd)[2];
 					   xlims(psp_spiro)[1] xlims(psp_spiro)[2]] |> 
					  x -> (minimum(x), maximum(x))
		
		p_c = plot(psp_copd, psp_spiro,
			fontfamily = myfont,
			xlims = psp_copd_x_lims,
			size = (640,380),
			layout = @layout [a b])
...
begin 
	...
	psp_avgdiff_x_lims = [xlims(p1_sp)[1] xlims(p1_sp)[2];
 				          xlims(p2_sp)[1] xlims(p2_sp)[2]] |> 
					      x -> (minimum(x), maximum(x))
	
	 p_d = plot(p1_sp, p2_sp, 
			fontfamily = myfont,
			xlims = psp_avgdiff_x_lims,
			size = (640,380),
			layout = @layout [a b])
...
# ╔═╡ 45d7456f-138e-4eb3-a4ec-500f427684d2
let 
	psp_xlims = [xlims(psp_copd)[1] xlims(psp_copd)[2];
			 	 xlims(psp_spiro)[1] xlims(psp_spiro)[2];
				 xlims(p1_sp)[1] xlims(p1_sp)[2];
			 	 xlims(p2_sp)[1] xlims(p2_sp)[2]] |> 
					x -> (minimum(x), maximum(x))
...
# ╔═╡ d40cc99f-cd63-4cf9-b784-fb955f8a39b5
begin
	#############
	# Functions #
	#############
	...
	function compute_ticks(gdf_sub_selection, y_rec)
	    vec_idx_tick = []
	    vec_subID_tick = []
	
	    for j in 1:length(gdf_sub_selection)
	        n_sub = size(gdf_sub_selection[j], 1)
	        
	        if n_sub <= 5
	            idx_sub_tick = [floor(Int, median(1:n_sub))]
	            subID_tick = [gdf_sub_selection[j].subID[idx_sub_tick[1]]]
	        else
	            num_ticks = floor(Int, n_sub / 5)
	            idx_sub_tick = [floor(Int, median((1 + (i - 1) * 5):(i * 5))) for i in 1:num_ticks]
	            subID_tick = [gdf_sub_selection[j].subID[idx] for idx in idx_sub_tick]
	        end
	        
	        vec_idx_tick = vcat(vec_idx_tick, idx_sub_tick .+ y_rec[j])
	        vec_subID_tick = vcat(vec_subID_tick, subID_tick)
	    end
	
	    return (vec_idx_tick.-0.5, vec_subID_tick)
	end		
	...
	sub_selection = levelsSub_copd[df_spiro_copd.idxcopd]
	df_sub_selection = DataFrame(
		subidx = collect(1:length(sub_selection)), 
		subID = sub_selection, 
		supID = map(x ->x[1:3],sub_selection)
	)
	gdf_sub_selection = groupby(df_sub_selection, :supID)
	width_sub = DataFrames.combine(gdf_sub_selection, nrow)
	y_rec = accumulate(+, vcat([0],width_sub.nrow))[1:end-1]

	y_sub_ticks = compute_ticks(gdf_sub_selection, y_rec)

	plt = plot(
		p1_sb, 
		title = "",
		xlabel = "COPDGene and SPIROMICS Average Sex: Female Effect Size",
		ylabel = "Subclass ID",
		yticks = y_sub_ticks,
		size = (580, 800),		
		left_margin = (10, :mm)
	)
	# w_rec = xlims(p1_sb)[2] -xlims(p1_sb)[1]
	# h_rec = width_sub.nrow[2]
	# y_rec = accumulate(+, vcat([0],width_sub.nrow))[1:end-1]
	for i in 1:2:8
		plot!(
			rectangle(
				xlims(p1_sb)[2] -xlims(p1_sb)[1], 
				width_sub.nrow[i], 
				xlims(p1_sb)[1], 
				y_rec[i]), 
			color = :grey, 
...
# ╔═╡ b1e58df3-6660-4211-a4fd-98a31820477a
md"""
The followling tables is more comprehensive showing the significant subpathway at a 95% confidence interval with effect size, T-statistics, P-values and FDR (False Discovery Rate):
"""

# ╔═╡ e87f2b5e-4e56-4bda-8075-d78cf2c76d75
begin 
	...
	df_sub_signif = DataFrame(
		...
		EffectSize = round.(est[idx_sub_sginif], digits = 2),
		CI = string.(
					"[",
...
begin 
	...
	# compute number of metabolites per module
	count_metabolites_module_copd = DataFrame(Int.(mZmdl_copd[:,1:end]), nameModules) |>
	x -> Matrix(x) |>
	x -> sum(x, dims = 1)
	df_copd_mdl_count = DataFrame(
		Module = nameModules,
		CountMetabolite= count_metabolites_module_copd |>vec
	) |> 
	x -> reverse(x)
	
	count_metabolites_module_spiro = DataFrame(Int.(mZmdl_spiro[:,1:end]), nameModules) |>
	x -> Matrix(x) |>
	x -> sum(x, dims = 1)
	df_spiro_mdl_count = DataFrame(
		Module = nameModules,
		CountMetabolite= count_metabolites_module_spiro |>vec
	)|> 
	x -> reverse(x)

	insertcols!(df_mdl, 2, :CountMetabolite => "(".*string.(df_copd_mdl_count.CountMetabolite).*", ".*string.(df_spiro_mdl_count.CountMetabolite).*")")
...
begin 
	if @isdefined copdZmdl
		...
		pmdl_copd_x_lims = [xlims(pmdl_copd)[1] xlims(pmdl_copd)[2];
 					       xlims(pmdl_spiro)[1] xlims(pmdl_spiro)[2]] |> 
					      x -> (minimum(x), maximum(x))

		p_a= plot(pmdl_copd, pmdl_spiro, 
			fontfamily = myfont,
			xlims = pmdl_copd_x_lims,
			size = (640,380),
			layout = @layout [a b])
...
begin 
	...
	pmdl_avgdiff_x_lims = [xlims(p1_mdl)[1] xlims(p1_mdl)[2];
				           xlims(p2_mdl)[1] xlims(p2_mdl)[2]] |> 
			    		  x -> (minimum(x), maximum(x))	
	
	p_b = plot(p1_mdl, p2_mdl, 
			fontfamily = myfont,
			xlims = pmdl_avgdiff_x_lims,
			size = (640,380),
			layout = @layout [a b])
...
# ╔═╡ 53e63e76-71a6-4b42-9e9b-c6379a684c35
let 	
	# l2 = @layout [
 #    a{0.9999w};  b{0.9999w} 
	# ]
	
	pmdl_xlims = [xlims(pmdl_copd)[1] xlims(pmdl_copd)[2];
			 	  xlims(pmdl_spiro)[1] xlims(pmdl_spiro)[2];
				  xlims(p1_mdl)[1] xlims(p1_mdl)[2];
			 	  xlims(p2_mdl)[1] xlims(p2_mdl)[2]] |> 
					x -> (minimum(x), maximum(x))
...
manifest_format = "2.0"
project_hash = "26163748da3300443835ce3644da78e6111cc1ab"
...
git-tree-sha1 = "f5e7d94dc2c4bd17c2492d26316d08762562d534"
