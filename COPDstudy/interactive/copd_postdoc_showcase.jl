...
begin
	if @isdefined copd
		...
		psp_copd = confidenceplot(
			...
			title = "COPDGene",
			titlefontsize = mytitlefontsize,
			right_margin = -10mm,xrotation = 25,
			# size = (800, 400),
		)
		...
		# 	vec(namesZsup_spiro),
			...
			# right_margin = 10mm,
			left_margin = -10mm,
			bottom_margin = 5mm,xrotation = 25
			# size = (800, 400),
		)
...
begin 
	...
	# SEcopd_sb = sqrt.(copdZsb.var[1:ncovar, df_spiro_copd.idxcopd]);
	# SEspiro_sb = sqrt.(spiroZsb.var[1:ncovar, df_spiro_copd.idxspiro]);
	# SEdiff_sb = sqrt.(SEcopd_sb.^2 .+ SEspiro_sb.^2);
	...
	confidenceplot(
		...
		xlabel = xCovarFig_sb*" Effect Size", legend = false,
		fontfamily = myfont,
		title = "Difference Effect Size between studies",
		# xlim = (-.32,.42) ,
		size = (500, 700),
	)
end
...
begin 
	df = sort(
		DataFrame(
			val = vec(permutedims(diffcoef_sb[idx_avg_diff_sub,:])),
	...
	confidenceplot(
		vec(permutedims(diffcoef_sb[idx_avg_diff_sub, df.idx])),
...
# ╔═╡ dd1c0304-4744-46d8-ac26-b1418becaf24
md"""
#### Weighted gene co-expression network analysis (WGCNA) modules based on metabolite sub class
"""
...
let 
	...
	# avgcoef_mdl = (spiroZmdl.coef[1:ncovar, :] .+ 
	# 			copdZmdl.coef[1:ncovar, :])./2;
