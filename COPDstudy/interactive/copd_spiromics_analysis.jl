...
@bind xCovariates MultiCheckBox(vcat(["Intercept"], names(copd.dfInd)[3:end]), 
...
begin 
	bttnModel
	contrasts = Dict(:Sex => EffectsCoding(base = sort(unique(copd.dfInd.Sex))[2]), 
	...
	if frml_c[] != ""
		...
		mXcopd = modelmatrix(formulaX, copd.dfInd, hints = contrasts)
	    mXspiro = modelmatrix(formulaX, spiro.dfInd, hints = contrasts) 
		...
		vFrmlNames = Vector{String}()
	 	...
	 	if frml_c[] == "1"
			...
			sch = schema(formulaX, copd.dfInd, contrasts)
...
begin
	if @isdefined dfTstatsZIcopd
		...
		dfMeanTst_copd = combine(gdf, Symbol(nameCovarFig) => mean => Symbol(nameCovarFig)) 
		sort!(dfMeanTst_copd, :SuperClassID);
	
		# var2vis = Symbol("Sex")
		p_copd = eval(Meta.parse("@df dfTstatsZIcopd dotplot(string.(:SuperClassID), :$(nameCovarFig), legend = false)"))
		...
		dfMeanTst_spiro = combine(gdf, Symbol(nameCovarFig) => mean => Symbol(nameCovarFig)) 
...
begin
	if @isdefined copd
		...
		psp_copd = confidenceplot(
			vec(permutedims(copdZsp.coef[idxCovarsup,Not(idx2rmv_copd)])), 
			levelsSup_copd[Not(idx2rmv_copd)],
			vec((copdZsp.ci[idxCovarsup,Not(idx2rmv_copd)])),
		...
		psp_spiro = confidenceplot(
			vec(permutedims(spiroZsp.coef[idxCovarsup,Not(idx2rmv_spiro)])),
			levelsSup_spiro[Not(idx2rmv_spiro)],
		# 	vec(namesZsup_spiro),
			vec((spiroZsp.ci[idxCovarsup,Not(idx2rmv_spiro)])),
			xlabel = xCovarFig*" Effect Size", legend = false,
			fontfamily = myfont,
			title = "SPIROMICS",
		)

		plot(psp_copd, psp_spiro, layout = @layout [a b])
...
begin 
	...
	ncovar = length(vFrmlNames)
	critical_level_sp = 1.96;
	
	diffcoef_sp = spiroZsp.coef[1:ncovar, Not(idx2rmv_spiro)] .- 
				copdZsp.coef[1:ncovar, Not(idx2rmv_copd)];

	avgcoef_sp = (spiroZsp.coef[1:ncovar, Not(idx2rmv_spiro)] .+ 
			copdZsp.coef[1:ncovar, Not(idx2rmv_copd)])./2;
	

	SEcopd_sp = sqrt.(copdZsp.var[1:ncovar, Not(idx2rmv_copd)]);
	SEspiro_sp = sqrt.(spiroZsp.var[1:ncovar, Not(idx2rmv_spiro)]);
	SEdiff_sp = sqrt.(SEcopd_sp.^2 .+ SEspiro_sp.^2);

	CIdiff_sp = SEdiff_sp.*critical_level_sp; 

	p1_sp = confidenceplot(
		vec(permutedims(diffcoef_sp[idxCovarsup,:])),
		levelsSup_spiro[Not(idx2rmv_spiro)],
		vec((CIdiff_sp[idxCovarsup, :])),
	...
	p2_sp = confidenceplot(
		...
		vec(permutedims(avgcoef_sp[idxCovarsup, :])),
		levelsSup_spiro[Not(idx2rmv_spiro)],
		vec((CIdiff_sp[idxCovarsup, :])),
	...
	plot(p2_sp, p1_sp, layout = @layout [a b])
...
if (@isdefined mXcopd)
	...
	############################################
	# Adjsust idx to compare similar sub class #
	############################################
	...
	dfcopd2 = filter(row -> row.nrow > 4,  combine(groupby(dfcopd[:, :], [:SubPathway]), nrow))
	
	dfspiro2 = filter(row -> row.nrow > 4,  combine(groupby(dfspiro[:, :], [:SubPathway]), nrow))
	...
	# keep unique  ID
	dfcopd = sort(
			combine(groupby(dfcopd[:, :], [:SubClassIDcopd]), last), :SubClassIDcopd
			)  
	insertcols!(dfcopd, 1, :idxcopd => 1:size(dfcopd, 1))
	dfspiro = sort(
			combine(groupby(dfspiro[:, :], [:SubClassIDspiro]), last), :SubClassIDspiro
			)  
...
if (@isdefined mXcopd)
	if radio_Zsb
		DataFrame(hcat(Int.(mZsub[:,1:end]), copd.dfRef.SubClassID[:]), vcat(levelsSub_copd, ["Class"]))
	else
...
begin 
	if @isdefined copdZsb
		...
		psb_copd = confidenceplot(
			vec(permutedims(copdZsb.coef[idxCovarsub, df_spiro_copd.idxcopd])), 
			levelsSub_copd[df_spiro_copd.idxcopd],
			vec((copdZsb.ci[idxCovarsub, df_spiro_copd.idxcopd])),
		...
		psb_spiro = confidenceplot(
			vec(permutedims(spiroZsb.coef[idxCovarsub, df_spiro_copd.idxspiro])), 
			# levelsSub_spiro[idx_spiro_intersub],
			levelsSub_copd[df_spiro_copd.idxcopd],
			vec((spiroZsb.ci[idxCovarsub, df_spiro_copd.idxspiro])),
			xlabel = xCovarFig_sb*" Effect Size", legend = false,
			fontfamily = myfont,
			title = "SPIROMICS",
			size=(800,700), 
		...
		# psb_copd = confidenceplot(
		# 	vec(permutedims(copdZsb.coef[idxCovarsub, :])), 
		# 	levelsSub_copd,
		# 	vec((copdZsb.ci[idxCovarsub, :])),
		# 	xlabel = xCovarFig*" Effect Size", legend = false,
		# 	fontfamily = myfont,
		# 	title = "COPDGene",
		# 	size=(800,700), 
...
# ╔═╡ 7fd47032-f034-47da-a33a-819fd70054a9
begin 
	diffcoef_sb = spiroZsb.coef[:, df_spiro_copd.idxspiro] .- 
				copdZsb.coef[:, df_spiro_copd.idxcopd];

	SEcopd_sb = sqrt.(copdZsb.var[:, df_spiro_copd.idxcopd]);
	SEspiro_sb = sqrt.(spiroZsb.var[:, df_spiro_copd.idxspiro]);
	SEdiff_sb = sqrt.(SEcopd_sb.^2 .+ SEspiro_sb.^2);

	CIdiff_sb = SEdiff_sb.*1.96; 

	confidenceplot(
		vec(permutedims(diffcoef_sb[idxCovarsub,:])),
		levelsSub_copd[df_spiro_copd.idxcopd],
		vec((CIdiff_sb[idxCovarsub, :])),
		xlabel = xCovarFig_sb*" Effect Size", legend = false,
		fontfamily = myfont,
		title = "Difference in Effect Size between Studies",
		# xlim = (-.32,.42) ,
		size = (500, 700),
	)
end
...
begin 
	df = sort(
		DataFrame(
			val = vec(permutedims(diffcoef_sb[idxCovarsub,:])),
			idx = 1:size(diffcoef_sb , 2),
	...
	confidenceplot(
		vec(permutedims(diffcoef_sb[idxCovarsub, df.idx])),
		levelsSub_copd[df_spiro_copd.idxcopd][df.idx],
		vec((CIdiff_sb[idxCovarsub, df.idx])),
		xlabel = xCovarFig_sb*" Effect Size", legend = false,
		fontfamily = myfont,
		title = "Difference in Effect Size between Studies",
		# xlim = (-.32,.42) ,
...
begin
	xcopd = vec(permutedims(diffcoef_sb[idxCovarsub, df.idx]));
	εcopd =  vec((CIdiff_sb[idxCovarsub, df.idx]));
	...
	if isempty(idxCritical)
		...
		latexify(df[idxCritical, [:idSubPathway, :SubPathway]];
...
begin 
	if @isdefined copdZmdl
		...
		pmdl_copd = confidenceplot(
			vec(permutedims(copdZmdl.coef[idxCovarmdl, :])), 
			nameModules,
			vec((copdZmdl.ci[idxCovarmdl, :])),
		...
		pmdl_spiro = confidenceplot(
			vec(permutedims(spiroZmdl.coef[idxCovarmdl, :])), 
			nameModules,
			vec((spiroZmdl.ci[idxCovarmdl, :])),
			xlabel = xCovarFig_mdl*" Effect Size", legend = false,
			fontfamily = myfont,
			title = "SPIROMICS",
			# size=(800,700), 
...
let 
	...
	p1 = confidenceplot(
		...
		vec(permutedims(diffcoef_mdl[idxCovarmdl, :])),
		nameModules[:],
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "18d5b028f34d7c5bac63e3095d69bedf9a7d03dc"
