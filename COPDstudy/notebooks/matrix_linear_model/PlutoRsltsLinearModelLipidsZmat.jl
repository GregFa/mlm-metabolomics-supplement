...
# ╔═╡ d406b9d5-2a7f-47b0-ae9b-59c60d9d008b
begin
	#############
	# Libraries #
	#############
...
# ╔═╡ 73308358-17be-4a59-885a-748deb454860
...
# ╔═╡ e367d9fe-4160-4707-ae91-267b80cc71d9
md"""
...
# ╔═╡ 29976570-1f03-4d22-9392-58559434ac23
md"""
...
@bind slct1 Select(covarSelection, default = "2")
...
begin
	...
	scatter(dfZraw.Total_DB .+ rand(Uniform(-a,a),length(dfZraw.Total_DB)),
		dfZraw.Total_C,
		zcolor = TstatZI[idxColXmat,:], colorbar = true, m = (:bluesreds, 0.8), 
		colorbar_title = string("T-statistics of ",namesX[idxColXmat]),
...
$(if rdZ == "Triglycerides"
	(@bind slct2 Select(["~ Total C*Total DB"=> "Continuous: Total C, Total DB, Total C:Total DB", 
...
begin 
	...
	classZ = Dict("~ Total C*Total DB"=>"Chain length and degree of unsaturation", 
			   	"factor(Total C, Total DB)"=>"Chain length and degree of unsaturation",
				"factor(Total C)"=>"Chainlength",
...
md"""
#### Plots of Matrix Linear Model regression coefficients
"""

# ╔═╡ 24303e11-aa87-4f88-b217-1834f7a6f94a
md"""
##### Effect Size
"""

# ╔═╡ 1161c4c2-5d32-42db-84e8-9d4dd3c13cb1
plot(transpose(CoefZ[:,1:end]), 
    label= namesX, xlabel= classZ[slct2], ylabel = "Effect size",
    legend =:outerright, lw = 2, marker = :circle, grid = false,
    xticks = (collect(1:size(Z)[2]), namesZ),xrotation = 45, size = (800,550),
...
# ╔═╡ 3af98e83-2235-4e8b-ba3b-b7e03e72a03f
md"""
##### T-statistics
"""

# ╔═╡ ab1c5053-dafa-4968-b848-67f2d368c1ec
begin
plot(transpose(TstatZ[:,1:end]), 
    label= namesX, xlabel= classZ[slct2], ylabel = "T-statistics",
    legend =:outerright, lw = 2, marker = :circle, grid = false,
    xticks = (collect(1:size(Z)[2]), namesZ),xrotation = 45, size = (800,550),
	bottom_margin = 10mm, format = :svg,
...
md"""
$(@bind slct3 Select(covarSelection, default = "2"))
"""
