...
# Filter fish younger than 8 and older than 11:

# + tags=[] jupyter={"outputs_hidden": true}
 dfIndividuals.Age = parse.(Int, string.(dfIndividuals.Age)) 
# -

idx_older = findall((dfIndividuals.Age .>=8) .&& (dfIndividuals.Age .<=11));

mH = hcat(dfIndividuals.Histological_Score,
    dfIndividuals.Histology_Liver,
    dfIndividuals.Histology_Swim_Bladder)
dfIndividuals.Histological_Score .= mean(mH, dims= 2);

dfIndividuals = dfIndividuals[idx_older, :];

select!(dfIndividuals, Not(["Histology_Liver","Histology_Swim_Bladder"]));
...
fileIndividuals = joinpath(@__DIR__,"..","..","data","processed","ST001052_ClinicalCovariates_old.csv");
...
fileDemog = joinpath(@__DIR__,"..","..","data","processed","Demog_old.csv");
...
fileMetabo = joinpath(@__DIR__,"..","..","data","processed","Metabo_old.csv");
