...
begin
	...
	using Plots.PlotMeasures, ColorSchemes, PlotlyBase, RecipesBase
...
dfMetTG = dfMet[:, vcat([:SampleID], Symbol.(dfRefTG.MetaboliteID))]
...
md"""
\* *Notes:*
*Vet score, PCV color, and histology score determine if a tilapia is healthy or pansteatitis-affected.*     
"""
...
bttnModel
contrasts = Dict(
	:Group => EffectsCoding(base=sort(unique(dfInd.Group), rev=true)[1]), 
	:Gender => EffectsCoding(base=sort(unique(dfInd.Gender))[1])
)
...
if frml_c[] != ""
	...
	vFrmlNames = Vector{String}()
 	...
 	if frml_c[] == "1"
		...
		vFrmlNames = ModelFrame(eval(Meta.parse(string("@formula(0 ~ ", frml_c[], ")"))), dfInd) |> coefnames
	end
...
# ╔═╡ 28e4a464-dea9-4c93-9a3a-98d7d8ac186a
md"""
### Z matrix
"""

# ╔═╡ 9a29b559-a42f-4039-a40a-893a494a2828
begin
if (@isdefined mX)
	###################
	# Identity Matrix #
...
# ╔═╡ 9f818f61-9c36-4754-93b4-4a3ad5b91c74
@bind zCovariates Radio(["Identity Matrix", "Super Class", "Sub Class"], 
						 default = "Identity Matrix")

# ╔═╡ 0dcba5eb-6931-4480-a053-98948e5a522e
begin
	if (@isdefined mX)
		if zCovariates == "Identity Matrix"
			first(dfTstatsZI, 3)
			ZI
		elseif zCovariates == "Super Class"
			size(TstatZsp)
			DataFrame(hcat(Int.(mZsup[:,1:end]), dfRef.super_class[:]), vcat(levelsSup, ["Class"]))
		elseif zCovariates == "Sub Class"
			DataFrame(hcat(Int.(mZsub[:,1:end]), dfRef.sub_class[:]), vcat(levelsSub, ["Class"]))
		end
	end
end
...
if @isdefined dfTstatsZI
	...
	eval(Meta.parse("@df dfTstatsZI dotplot(string.(:super_class), :$(nameCovarFig), label = \"Metbolite\", legend = false)"))
...
begin
	if @isdefined TstatZsp
		...
		confidenceplot(vec(permutedims(CoefZsp[idxCovar,:])), 
				   vec(namesZsup),
				   vec(permutedims(CIZsp[idxCovar,:])),
				   xlabel = xCovarFig*" Effect Size", legend = false,
		)
	end
end

# ╔═╡ 9726edf0-1e2b-401f-a17b-f2ac952b0dac
md"""
### Summary T-statistic heatmap
"""
...
# ╔═╡ 33c567da-ffa8-47b3-abfe-cd9a7ad3e58f
...
if @isdefined vPseudoFrmlNames
	@bind xCovarFigsub Select(vFrmlNames)
end
...
if @isdefined dfTstatsZI
	...
	eval(Meta.parse("@df dfTstatsZI dotplot(string.(:sub_class), :$(nameCovarFigsub), label = \"Metbolite\", legend = false)"))
	...
	hline!([0], color= :red, label = "", bottom_margin = 10mm,
		foreground_color_legend = nothing, # remove box of legend
		xlabel = "Super class", ylabel = string("T-statistics ", nameCovarFigsub), 
		title = "T-statistics per class")
end
end
...
begin
	if @isdefined TstatZsb
		...
		idxCovarsub = findall(vPseudoFrmlNames .== fix_covar_name(xCovarFigsub))
		namesZsub = string.(permutedims(levelsSub[1:end]))
		confidenceplot(vec(permutedims(CoefZsb[idxCovarsub,:])), 
				   vec(namesZsub),
				   vec(permutedims(CIZsb[idxCovarsub,:])),
				   xlabel = xCovarFigsub*" Effect Size", legend = false,
		)
	end
end
...
if @isdefined vPseudoFrmlNames
	@bind xCovarFigTG Select(vFrmlNames)
end
...
if (@isdefined xCovarFigTG)	
	...
	Plots.scatter(dfRefTG.Total_DB .+ rand(Uniform(-a,a),length(dfRefTG.Total_DB)),
		dfRefTG.Total_C,
...
if @isdefined vPseudoFrmlNames
	@bind xCovarFigTGdb Select(vFrmlNames)
end
...
if (@isdefined mX)	
	...
	# Generate Z matrix
	mZdb = modelmatrix(@formula(y ~ 0 + Total_DB).rhs, 
						dfRefTG₂, 
						hints = Dict(:Total_DB => StatsModels.FullDummyCoding()));
	# true indicates correct design matrix
	levelsDB[(mapslices(x ->findall(x .== 1) , mZdb, dims = [2]))[:]] == string.(dfRefTG₂.Total_DB)
	levelsDB = "DB_".*levelsDB;
	levelsDB[end] = "DB_≥_"*string(max_db);

	CoefZdb, CIZdb, TstatZdb, varZdb = getCoefs(mYTG, mX, mZdb);
end
end;
...
begin
	if @isdefined TstatZdb
		...
		confidenceplot(vec(permutedims(CoefZdb[idxCovarTGdb,:])), 
				   vec(namesZdb),
...
# ╔═╡ 42496b70-daf1-46cc-bf33-7f9f46b0ebb5
 mlmheatmap(permutedims(TstatZdb[2:end, :]), permutedims(TstatZdb[2:end,:]), 
	    xticks = (collect(1:size(mZdb)[1]), namesXtbl), xrotation = 25,
	    yticks = (collect(1:size(mZdb)[2]), levelsDB), tickfontfamily = myfont ,
	    color = cgrad(:bluesreds,[0.1, 0.3, 0.7, 0.9], alpha = 0.8), cbar = true,
	 	clims = (-3, 3), colorbar_title = "T-statistics",
	    annotationargs = (10, "Arial", :lightgrey), 
	    linecolor = :white , linewidth = 2, margins=8mm)
...
md"""
### Association at the carbon number level
"""

# ╔═╡ fdeece47-c239-42d5-b06e-7e289edaef72
if @isdefined vPseudoFrmlNames
	@bind xCovarFigTGc Select(vFrmlNames)
end
...
if (@isdefined mX)	
	...
	levelsC = ["CC_≤_50", "50_<_CC_≤_55", "55_<_CC_≤_60",
			   "60_<_CC_≤_65","CC_>_65"];

	# Generate Z matrix
	mZc = modelmatrix(@formula(y ~ 0 + Total_C).rhs, 
						dfRefTG₂, 
						hints = Dict(:Total_C => StatsModels.FullDummyCoding()));

	CoefZc, CIZc, TstatZc, varZc = getCoefs(mYTG, mX, mZc);
end
end;
...
begin
	if @isdefined TstatZc
		...
		confidenceplot(vec(permutedims(CoefZc[idxCovarTGc,:])), 
				   vec(namesZc),
...
# ╔═╡ 3fe326da-d657-4cf0-b161-0b921798a7ce
 mlmheatmap(permutedims(TstatZc[2:end, :]), permutedims(TstatZc[2:end,:]), 
	    xticks = (collect(1:size(mZc)[1]), namesXtbl), xrotation = 25,
	    yticks = (collect(1:size(mZc)[2]), levelsC), tickfontfamily = myfont ,
	    color = cgrad(:bluesreds,[0.1, 0.3, 0.7, 0.9], alpha = 0.8), cbar = true,
	 	clims = (-3, 3), colorbar_title = "T-statistics",
	    annotationargs = (10, "Arial", :lightgrey), 
	    linecolor = :white , linewidth = 2, margins=8mm)
...
md"""
### Association at the carbon number and unsaturation level
"""

# ╔═╡ c3a2675b-3276-48f4-8a9a-3851194c1679
if @isdefined vPseudoFrmlNames
	@bind xCovarFigTGcdb Select(vFrmlNames)
end
...
if (@isdefined mX)	
	...
	dfRefTG4.Total_C = float.(dfRefTG4.Total_C)./6 # per six unit Carbon
	dfRefTG4.Total_DB = float.(dfRefTG4.Total_DB)
...
# ╔═╡ f2a36108-2156-4997-ba52-734c4f07732d
 mlmheatmap(permutedims(TstatZcdb[2:end, 2:end]), permutedims(TstatZcdb[2:end,2:end]), 
	    xticks = (collect(1:size(mZcdb)[1]), namesXtbl), xrotation = 25,
	    yticks = (collect(1:size(mZcdb,2)-1), levelsC_DB[2:end]), tickfontfamily = myfont ,
	    color = cgrad(:bluesreds,[0.1, 0.3, 0.7, 0.9], alpha = 0.8), cbar = true,
	 	clims = (-3, 3), colorbar_title = "T-statistics",
	    annotationargs = (10, "Arial", :lightgrey), 
	    linecolor = :white , linewidth = 2, margins=8mm)
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "9013f4e97f421f7cd682892381136680ada741a5"
