...
fileIndividuals = joinpath(@__DIR__,"..","data","processed",
							"ST001052_ClinicalCovariates_old.csv");
...
# ╔═╡ af73aa6d-284e-4b9a-8ffe-3f822e5e8147
dfMetTG

# ╔═╡ 3937cd7f-0fc0-401a-8b63-afa83838f37f
begin
 A = Matrix(dfMetTG[:,2:end]);
 # dt  = fit(ZScoreTransform, A; dims=1)
end

# ╔═╡ 153eb781-4005-44ae-8f02-773db8db178a
begin
	X = [0.0 -0.5 0.5; 0.0 1.0 2.0]
	fit(ZScoreTransform, X, dims=2)

end

# ╔═╡ 9da41e52-1490-4457-a1bb-81fd8f956738
typeof(parse.(Float64,A[:, 96]))

# ╔═╡ 3cd96225-0193-451a-834c-25237a9fae26
mT = typeof.(A[:,:]);

# ╔═╡ 937eeeba-73cd-46eb-9792-e1bb56515f03
mBool = (mT[:,:] .== Float64) 

# ╔═╡ 55e83f2f-f7ab-41ca-894a-48fa07e15098
sum(mBool,dims = 1)[1,96]

# ╔═╡ f6e04560-8611-4c6f-8ac2-0a768065e285
findall(sum(mBool,dims = 1).!= 26)

# ╔═╡ 08500c27-fde5-4100-b751-927607531625
...
@bind xCovariates MultiCheckBox(vcat(["Intercept"], names(dfInd)[2:end]),
						default = ["Intercept", "Histological_Score"])
...
if @isdefined(TstatZsp)
	mlmheatmap(permutedims(TstatZsp[2:end, :]), permutedims(TstatZsp[2:end,:]), 
		    ...
		    yticks = (collect(1:size(mZsup)[2]), levelsSup),
...
if (@isdefined mX)	
	...
	levelsC = ["Total Carbon ≤ 50", "50 < Total Carbon ≤ 55","55 < Total Carbon ≤ 60","60 < Total Carbon ≤ 65","Total Carbon > 65", 
		"Double Bond 1", "Double Bond 2", "Double Bond 3"];
	levelsC = ["Intercept", "Total Carbon ≤ 50", "50 < Total Carbon ≤ 55","55 < Total Carbon ≤ 60","60 < Total Carbon ≤ 65", 
		"Double Bond 1", "Double Bond 2", "Double Bond 3"];

	dfRefTG₃.Total_DB = dfRefTG₃.Total_DB;#.*1.0;
	# dfRefTG₃.Total_C = string.(dfRefTG₃.Total_C)
	# Generate Z matrix
	mZc = modelmatrix(@formula(y ~ 1 + Total_C +Total_DB).rhs, 
						dfRefTG₃, 
						hints = Dict(
...
# ╔═╡ e7af3065-9354-486a-a471-c01617283606
mZc
...
if @isdefined(TstatZcdb)
 	mlmheatmap(permutedims(TstatZcdb[2:end, 2:end]),
		permutedims(TstatZcdb[2:end,2:end]), 
	    xticks = (collect(1:size(mZcdb)[1]), namesXtbl), xrotation = 25,
	    yticks = (collect(1:size(mZcdb,2)-1), levelsC_DB[2:end]), 
